...
| **Media** | A photo or video **file, decoupled from any one issue** (phase 2) so one file can back several reports. Dated photos+videos are the evidence trail official channels don't keep public. `kind` drives rendering; the copy always lives on our own storage, even when pulled in by a social-media import. **Ownership:** direct uploads belong to their uploader (a logged-in user, else an anonymous `owner_token` kept in session data, which survives login's session-key rotation) and only the owner may reuse them; imported media is `origin=import` with ownership left dangling for admins to assign. |
